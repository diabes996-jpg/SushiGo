/* ----------------------------- Datos iniciales ----------------------------- */
  ...
  function renderRestaurantes(contenedor){
    ...
    restaurantes.forEach(r=>{
      ...
      d.innerHTML = `<h3>${escapeHtml(r.name)}</h3><div class="muted">${escapeHtml(r.direccion)}</div><div style="margin-top:8px">
	  																<a href="tel:${r.phone}" class="small">${escapeHtml(r.phone)}</a></div>`;
  ...
  function renderContadorCarrito(){ 
	const contador = carrito.items.reduce((s,i)=>s+i.qty,0); 
	document.getElementById("contador_carrito").textContent = contador; 
	document.getElementById("carrito_vacio").textContent = contador===0? "Vacío" : `${contador} producto(s)`; 
  }

  function renderElementosCarrito(){
    const list = document.getElementById("productos_carrito"); 
	list.innerHTML = "";
	
    if(carrito.items.length===0){ 
		list.innerHTML="<div class='muted'>No has añadido productos</div>"; 
		updateTotals(); return; }

    carrito.items.forEach(it=>{
      const p = encontrarProducto(it.productId);
      const node = document.createElement("div"); 
	  node.className="carrito-elemento";
      node.innerHTML = `
      ...
      node.querySelector("input[type=number]").addEventListener("change", e=>{ 
  ...
  function seededRandomIndex(length, seed){ 
	if(length<=0) return 0; const h = cyrb53(seed||String(Date.now())); return Math.abs(h) % length; 
  }
